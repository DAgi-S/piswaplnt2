...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    try {
        ...
        $required_fields = array('id', 'payment_date', 'paid_by', 'currency', 'amount');
        ...
        $sql = "UPDATE gps_payment_followup SET 
                payment_date = ?,
                paid_by = ?,
                currency = ?,
                amount = ?,
        ...
        if ($stmt->execute()) {
            if ($stmt->affected_rows > 0) {
                ...
                $response['messages'][] = "Payment follow-up updated successfully";
            } else {
                throw new Exception("No changes were made or record not found.");
            }
        } else {
            throw new Exception("Error updating payment follow-up: " . $stmt->error);
        }
        ...
        // Delete uploaded file if record update fails
        if (isset($target_file) && file_exists($target_file) && $payment_image !== $old_image) {
            unlink($target_file);
        }
    }
} else {
