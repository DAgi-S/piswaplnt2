<?php
// This is a direct database adjustment script that bypasses the API
// to fix overpaid sales
...
<html>
<head>
    <title>Direct Overpayment Fix</title>
    <style>
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['sale_id'])) {
    ...
    try {
        ...
        if ($action === 'fix_directly') {
            ...
            if ($accountResult->num_rows === 0) {
                // Get account table structure to verify field names
                ...
                // Create account insertion query based on available fields
                $insertFields = [];
                $insertValues = [];
                $bindTypes = '';
                $bindParams = [];
                ...
                $defaultAccountData = [
                    ...
                    'account_platform' => 'Default Account',
                ...
                // Build dynamic query
                $createAccountQuery = "INSERT INTO accounts (" . implode(', ', $insertFields) . 
                                     ") VALUES (" . implode(', ', $insertValues) . ")";
                
                $createAccountStmt = $connect->prepare($createAccountQuery);
                
                if (!$createAccountStmt) {
                    throw new Exception("Error preparing account creation: " . $connect->error);
                }
                ...
                if (!empty($bindParams)) {
                    ...
                    // Construct parameter binding
                    $params = array_merge([$bindTypes], $bindParamsRefs);
                    call_user_func_array([$createAccountStmt, 'bind_param'], $params);
                }
                
                if (!$createAccountStmt->execute()) {
                    throw new Exception("Could not create default account: " . $createAccountStmt->error);
                }
                
                $accountId = $connect->insert_id;
                
                echo '<div class="alert alert-info">
                    <p>Created default account for adjustment as none was found.</p>
                </div>';
            } else {
            ...
            // Fix 1: Update the sales_orders table directly
            ...
            // Fix 2: Add a negative payment record to balance things out
            $query = "INSERT INTO sales_payments (
                        ...
                        created_by,
                        created_at
                    ) VALUES (?, NOW(), 'Adjustment', ?, 'DIRECT-FIX-" . date('YmdHis') . "', 'Direct balance adjustment', 'approved', ?, ?, NOW())";
            
            $stmt = $connect->prepare($query);
            $negativeAmount = -1 * $overpayment;
            $userId = $_SESSION['userId'];
            $stmt->bind_param("idii", $saleId, $negativeAmount, $accountId, $userId);
            $stmt->execute();
            ...
            echo '<div class="alert alert-success">
                ...
                <p>A negative payment entry of ' . number_format($negativeAmount, 2) . ' has been added to balance the account.</p>
        ...
        $query = "SELECT payment_date, amount, payment_method, reference_number, notes, status 
                 FROM sales_payments 
    ...
    if ($result->num_rows > 0) {
        echo '<div class="alert alert-warning">
            <strong>Warning:</strong> This tool makes direct database changes. Use with caution!
        </div>
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            echo '<tr>
                ...
                <td>
                    ...
                    &nbsp;
                    <form method="post" action="" style="display: inline;" onsubmit="return confirm(\'Are you sure you want to fix this overpayment?\');">
                        <input type="hidden" name="sale_id" value="' . $row['id'] . '">
