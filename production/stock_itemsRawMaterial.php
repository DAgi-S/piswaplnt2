...
FROM raw_materials rm
LEFT JOIN warehouse_stock ws ON ws.item_id = rm.id 
    AND ws.item_type = 'raw_material'
WHERE rm.id = ?";
...
$totalPurchased = $stockResult['total_purchased'] ?? 0;
$totalConsumed = $stockResult['total_consumed'] ?? 0;
$averageCost = $stockResult['cost_per_unit'] ?? 0;

// Get stock movements for history
$movementsSql = "SELECT 
    DATE_FORMAT(rmm.created_at, '%Y-%m-%d %H:%i:%s') as movement_date,
    rmm.movement_type,
    CONCAT(FORMAT(rmm.quantity, 2), ' ', CONVERT(rm.unit USING utf8mb4) COLLATE utf8mb4_general_ci) as quantity_display,
    w.name as warehouse_name,
    CASE 
        WHEN rmm.reference_type = 'purchase' THEN (
            SELECT CONCAT(
                'PO #PO-', 
                ...
                LPAD(p.id, 3, '0'),
                ' - ',
                CONVERT(s.company_name USING utf8mb4) COLLATE utf8mb4_general_ci,
                ' (Rate: ',
                FORMAT(pi.rate, 2),
                '/unit, Total: ',
                FORMAT(pi.total, 2),
            ...
            WHERE p.id = rmm.reference_id AND pi.raw_material_id = rm.id
            LIMIT 1
        )
        ELSE CONVERT(rmm.notes USING utf8mb4) COLLATE utf8mb4_general_ci
    END as reference_info,
    CONVERT(rmm.notes USING utf8mb4) COLLATE utf8mb4_general_ci as details,
    COALESCE(CONVERT(u.username USING utf8mb4) COLLATE utf8mb4_general_ci, 'system') as created_by,
    rmm.quantity as raw_quantity
FROM raw_material_movements rmm
LEFT JOIN raw_materials rm ON rmm.material_id = rm.id
LEFT JOIN users u ON rmm.created_by = u.user_id
LEFT JOIN warehouses w ON rmm.warehouse_id = w.id
WHERE rmm.material_id = ? 
        ...
        <div class="panel panel-default">
            <div class="panel-heading">
                <div class="page-heading">
                    <i class="fa fa-info-circle"></i> Raw Material Details - <?php echo htmlspecialchars($itemDetails['name']); ?>
                    ...
                    <div class="col-md-6">
                        <div class="stat-card">
                            ...
                            <table class="table table-bordered">
                                ...
                                <tr>
                                    <th>Cost Per Unit</th>
                                    <td><?php echo htmlspecialchars(number_format($itemDetails['cost_per_unit'], 2)); ?></td>
                                </tr>
                            </table>
                        </div>
                    </div>
                                        ...
                                        <?php if (!empty($movements)): ?>
                                            <?php foreach ($movements as $movement): ?>
                                                <tr>
                                                    ...
                                                    <td>
                                                        <?php 
                                                        if (!empty($movement['reference_info']) && strpos($movement['reference_info'], 'PO #') === 0) {
                                                            $poRef = substr($movement['reference_info'], 0, strpos($movement['reference_info'], ' - '));
                                                            echo 'Purchase Order: ' . htmlspecialchars($poRef);
                                                        } else {
                                                            echo '-';
                                                        }
                                                        ?>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const maxValue = Math.max(data.purchased, data.current, data.consumed);
    ...
    const nodes = [
        { id: 'purchase', label: 'Purchased', value: data.purchased, x: nodeSpacing, color: '#28a745', icon: '⬇️' },
        { id: 'inventory', label: 'Current Stock', value: data.current, x: 2 * nodeSpacing, color: '#007bff', icon: '📦' },
        { id: 'consumed', label: 'Consumed', value: data.consumed, x: 3 * nodeSpacing, color: '#dc3545', icon: '⬆️' }
    ];
