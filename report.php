...
$(document).ready(function() {
    ...
    function sendEmailReport() {
        ...
        $.ajax({
            url: 'php_action/sendOrderReport.php',
            type: 'POST',
            data: $("#getOrderReportForm").serialize(),
            dataType: 'json',
            success: function(response) {
    ...
    function handleReportSuccess(response) {
        if(response.success) {
            if(response.html.includes('no-sales-message')) {
                // No sales case
                $('#previewContent').html(response.html);
                $('#messages').html('<div class="alert alert-info">Report generated successfully - No sales found for the selected period</div>');
            } else {
                // Normal case with sales data
                $('#previewContent').html(response.html);
